...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <motion.div
        style={{ scaleX }}
        className="absolute inset-x-0 bottom-0 h-0.5 origin-left bg-gradient-to-r from-[#059669] via-[#10B981] to-[#34D399]"
      />
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-2 md:px-6">
        <Link to="/" className="ember-logo group flex items-center gap-2">
          ...
          <span className="ember-word bg-gradient-to-r from-[#059669] via-[#10B981] to-[#34D399] bg-clip-text font-display text-base uppercase tracking-wider text-transparent">
        ...
        <nav className="hidden flex-1 items-center justify-center gap-1 md:flex">
          {categories.map((c) => (
            ...
            >
              ...
              <span className="absolute inset-x-5 -bottom-1 h-0.5 w-0 bg-gradient-to-r from-nav-glow-start to-nav-glow-end opacity-0 transition-all duration-300 group-hover:w-[calc(100%-40px)] group-hover:opacity-100 group-[.active]:inset-x-4 group-[.active]:w-auto group-[.active]:opacity-100 group-[.active]:shadow-[0_0_12px_rgba(16,185,129,0.6)]" />
            </Link>
          ))}
        </nav>
        ...
        <form onSubmit={handleSearch} className="ml-auto hidden md:block">
          <div className="ember-search relative">
            ...
            <input
              ...
              className="w-48 rounded-[7px] border border-[#059669] bg-card py-1.5 pl-9 pr-3 text-sm shadow-[0_0_12px_rgba(16,185,129,0.55)] placeholder:text-muted-foreground transition-shadow focus:border-[#10B981] focus:shadow-[0_0_18px_rgba(16,185,129,0.8)] focus:outline-none"
            />
          </div>
        </form>
            ...
            <form onSubmit={handleSearch}>
              <div className="ember-search relative">
                ...
                <input
                  ...
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Buscar artigos..."
                  className="w-full rounded-[7px] border border-[#059669] bg-card py-2 pl-9 pr-3 text-sm shadow-[0_0_12px_rgba(16,185,129,0.55)] placeholder:text-muted-foreground transition-shadow focus:border-[#10B981] focus:shadow-[0_0_18px_rgba(16,185,129,0.8)] focus:outline-none"
                />
              </div>
            </form>
